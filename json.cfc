...
<cfcomponent displayname="JSON" output="No">
				...
				<cfelseif (char EQ "," AND NOT inQuotes AND nestingLevel EQ 0) OR i EQ Len(_data)+1>
					...
					<cfelseif dataType EQ "struct" OR dataType EQ "query">
						...
						<cfif dataType EQ "struct">
							<cfset StructInsert( st, structKey, decode(structVal), true ) />
							<!--- <cfset arrayappend( ar, decode(dataStr) ) /> --->
	...
	<cffunction name="encode" access="remote" returntype="string" output="No"
		...
		<cfargument name="allKeyCase" type="string" required="No" default="" > <!-- 'upper' 'lower' or blank -->
		...
		<cfelseif IsArray(_data)>
			...
			<cfloop from="1" to="#ArrayLen(_data)#" index="i">
				<cfset tempVal = encode( _data[i], arguments.queryFormat, arguments.queryKeyCase, arguments.stringNumbers, arguments.formatDates, arguments.columnListFormat,arguments.allKeyCase ) />
		...
		<cfelseif IsStruct(_data)>
			...
			<cfloop from="1" to="#ArrayLen(arKeys)#" index="i">
				<cfset tempVal = encode( _data[ arKeys[i] ], arguments.queryFormat, arguments.queryKeyCase, arguments.stringNumbers, arguments.formatDates, arguments.columnListFormat,arguments.allKeyCase ) />
				<cfif dJSONString.toString() EQ "">
					<cfset dJSONString.append('"' & caseify(arKeys[i],arguments.allKeyCase) & '":' & tempVal) />
				<cfelse>
					<cfset dJSONString.append("," & '"' & caseify(arKeys[i],arguments.allKeyCase) & '":' & tempVal) />
				</cfif>
			</cfloop>
			...
			<cfif arguments.queryFormat EQ "query">
				...
				<cfloop list="#_data.columnlist#" delimiters="," index="column">
					...
					<cfloop from="1" to="#_data.recordcount#" index="i">
						...
						<cfset tempVal = encode( _data[column][i], arguments.queryFormat, arguments.queryKeyCase, arguments.stringNumbers, arguments.formatDates, arguments.columnListFormat,arguments.allKeyCase ) />
			...
			<cfelse>
				...
				<cfloop query="_data">
					...
					<cfloop list="#columnlist#" delimiters="," index="column">
						<cfset tempVal = encode( _data[column][CurrentRow], arguments.queryFormat, arguments.queryKeyCase, arguments.stringNumbers, arguments.formatDates, arguments.columnListFormat, arguments.allKeyCase ) />
	...
	<cffunction name="caseify" returntype="Any">
		<cfargument name="inString" type="string" required="true">
		<cfargument name="tocase" type="string" required="false" default="">
		<cfset var retval = arguments.inString>
		<cfswitch expression="#arguments.tocase#">
			<cfcase value="upper"><cfset retval = ucase(retval)></cfcase>
			<cfcase value="lower"><cfset retval = lcase(retval)></cfcase>
		</cfswitch>
		<cfreturn retval>
	</cffunction>
